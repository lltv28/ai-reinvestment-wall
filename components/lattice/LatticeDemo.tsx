...
import { STAGE_H, STAGE_W, useFitStage, useRecordingChrome } from '@/lib/adStage';
...
export function LatticeDemo({ mode = 'ads' }: { mode?: ReinvestmentMode }) {
  ...
  const dependencies = useMemo<VisualizerDependencies>(
    () => ({ onReinvestmentUpdate: setFrame, reinvestmentMode: mode }),
    [mode],
  );
